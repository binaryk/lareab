...
class FirmeOrganizatieController extends \BaseController
{   
    public function getFirme()
    {
        $firme = DB::select("SELECT 
            ent.id_entitate, 
            ent.denumire, 
            ent.cif,
            ent.adresa, 
        ...
        return View::make('entitate::firme_organizatie.list')->with('firme', $firme);
    }

    public function getAddFirma()
    {       
        return View::make('entitate::firme_organizatie.add');
    }

    public function postAddFirma()
    {
        $rules  = array(
            'denumire' => 'required',
            'num_ord_rc' => 'required'
        ...
        else
        {
            ...
            return Redirect::route('firme_organizatie_list');
        }
    }

    public function getEditFirma($id)
    {        
        $firma = DB::select("SELECT 
            ent.id_entitate, 
            ent.denumire, 
            ent.cif,
            ent.norc,
            ...
            AND ent.id_entitate = :id",
            array('id' => $id));
        return View::make('entitate::firme_organizatie.edit')->with('firma', $firma[0]);
    }

    public function postEditFirma($id_entitate)
    {
        $rules  = array(
            'denumire' => 'required',
            'num_ord_rc' => 'required'
    ...
    public function postDeleteFirma()
    {        
        if(Request::ajax()) {
            if( Session::token() === Input::get( '_token' ) ) {
                $id = Input::get('id_entitate');
                DB::table('entitate')->where('id_entitate', $id)->update(array(
                    'logical_delete' => 1));
                return $id;
            }
        }
