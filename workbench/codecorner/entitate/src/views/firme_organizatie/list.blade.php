               ...
               <div class="panel-heading">
                   Firme ce apartin organizatiei
                    <div class="pull-right">                      
                        ...
                        <a href="{{ URL::route('firme_organizatie_add') }}"><i class="fa fa-plus-circle fa-fw"></i> Nou</a>                      
                    </div>
               </div>
                      ...
                      <table class="table table-striped table-bordered table-hover" id="dataTables-firme_organizatie">
                        ...
                        <tbody>                             
                          @foreach ($firme as $firma)
                            <tr data-id="{{ $firma->id_entitate }}">
                              <td class="text-center">{{ $firma->denumire }}</td>
                              <td class="text-center">{{ $firma->cif }}</td>
                              <td>{{ $firma->localitate }}</td>
                              <td class="text-center">{{ $firma->judet }}</td>
                              <td class="text-center">{{ $firma->adresa }}</td>
                              <td>{{ $firma->cod_postal }}</td>
                              <td>{{ $firma->telefon }}</td>
                              <td>{{ $firma->fax }}</td>                                     
                              <td class="center action-buttons">
                                <a href="{{ URL::route('firme_organizatie_edit', $firma->id_entitate) }}">
                                  <i class="fa fa-pencil-square-o" 
                                  title="Vizualizeaza sau modifica"></i>
                                </a>
                                <a href="#"><i class="fa fa-trash-o" title="Sterge"></i></a>
                                <a href="{{ URL::route('reprezentant_legal_list_entitate', [$firma->id_entitate, $firma->denumire]) }}">
                                  <i class="fa fa-male" title="Reprezentanti legali"></i>
                                </a>
                                <a href="{{ URL::route('personal_entitate_list', [$firma->id_entitate, $firma->denumire]) }}"><i class="fa fa-users" title="Personal"></i></a>
                              </td>                                  
                            </tr>
                          @endforeach                             
                        </tbody>
